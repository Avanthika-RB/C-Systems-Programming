...
void *__sha256_finish_ctx(struct sha256_ctx *ctx, void *resbuf) {
    ...
    ctx->buffer32[(bytes + pad) / 4] =
        SWAP((ctx->total[TOTAL64_high] << 3) | (ctx->total[TOTAL64_low] >> 29));
#endif
...
void __sha256_process_bytes(const void *buffer, size_t len,
...
#define UNALIGNED_P(p) (((uintptr_t)p) % sizeof(uint32_t) != 0)
        ...
        if (UNALIGNED_P(buffer))
            while (len > 64) {
                __sha256_process_block(memcpy(ctx->buffer, buffer, 64), 64,
                                       ctx);
